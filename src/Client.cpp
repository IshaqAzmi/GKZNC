...
void CClient::ReadLine(const CString& sData) {
	...
	if (!IsAttached()) { // The following commands happen before authentication with ZNC
		if (sCommand.Equals("PASS")) {
			...
			if (m_bGotPass) {
				...
				PutClient(":*.GeekBouncer.co.uk NOTICE AUTH :*** "
	...
	if (sCommand.Equals("ZNC")) {
		...
		if (sLine.length() >= 5)
			PutClient(":*.GeekBouncer.co.uk PONG *.GeekBouncer.co.uk " + sLine.substr(5));
		else
			PutClient(":*.GeekBouncer.co.uk PONG *.GeekBouncer.co.uk");
...
void CClient::RefuseLogin(const CString& sReason) {
	...
	PutClient(":*.GeekBouncer.co.uk 464 " + GetNick() + " :" + sReason);
...
void CClient::AcceptLogin(CUser& User) {
	...
	if (!m_sNetwork.empty()) {
		...
		if (m_pNetwork && m_pUser->GetNetworks().size() > 1) {
			PutStatusNotice("PROBLEM? NEED HELP? Type /ZNC mode support to enter our support channel. A GeekBouncer Staff member will assist you.");
		}
	} else {
...
{
	PutClient(":*.GeekBouncer.co.uk CAP " + GetNick() + " " + sResponse);
}
