...
#define TWITTER_CONSUMER_KEY "ldFYf02RtSWi1ZQy1W5ww"
#define TWITTER_CONSUMER_SECRET "UIBvDdf8sPkl6gP8wSI41ljlSRGf67Eu3ICYU5E"
		...
		if(uResponseCode == 200)
		{
			...
			if(!pMod->m_token.empty() && !pMod->m_tokenSecret.empty())
			{
				...
				else
				{
					...
					pMod->PutModule("Please open this page in your browser to authorize GeekBouncer: http://twitter.com/oauth/authorize?oauth_callback=oob&oauth_token=" + pMod->m_token);
					pMod->PutModule("If you allowed GeekBouncer access, now enter the PIN code from the web page!");
				}
			}
			else
			{
